...
This component comes with 3 rendering modes (or shadeers):
...
Sethealth API makes writing this volumetric shaders extremelly easy by abstracting aways all the setup required to get 
...
A fragment shader is a small program that runs for **each** pixel, taking variables (uniforms), textures and the screen position as input and returning a color (vec4) as output.
...
## Built-in functions
Sethealth provides built-in functions to create your own volumetric ray caster on top of the medical data very easy. Since you can focus in writing the
...
Returns the normalized [normal vector](https://en.wikipedia.org/wiki/Normal_(geometry)) a the surface at the specified point.
...
It uses the provided "colormap" to the component, resolving a volume value to a vector color (RGBA).

### vec4 readColor(vec3 cursor)
This component uses `readVolume()` and `readColormap()` under the hood to return the resolve the color of
a specific 3D point using the provided colormap.
